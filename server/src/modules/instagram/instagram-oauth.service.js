...
class InstagramOAuthService {
  ...
  async handleCallback(code, clientId) {
    ...
    const tokenData = {
      ...
      token_expires_at: new Date(Date.now() + 10 * 365 * 24 * 60 * 60 * 1000), // effectively never
  ...
  async refreshToken(clientId) {
    ...
    // Validate the token is still working and update heartbeat.
  ...
  async getTokensExpiringWithin(days) {
    // Page tokens don't expire, but we still check active tokens
    // for validation purposes (the refresh job validates them)
  ...
  async _exchangeCode(code) {
    ...
    const url = `${GRAPH_URL}/oauth/access_token?${params.toString()}`;
    logger.info('Exchanging code for token', { url: url.replace(/client_secret=[^&]+/, 'client_secret=***').replace(/code=[^&]+/, 'code=***') });
    ...
    logger.info('Code exchange successful', { hasToken: !!data.access_token, tokenPrefix: data.access_token?.slice(0, 4) });
  ...
  async _exchangeForLongLived(shortToken) {
    ...
    const data = await res.json();
    logger.info('Long-lived token obtained', { expiresIn: data.expires_in, tokenPrefix: data.access_token?.slice(0, 4) });
    return data.access_token;
  }

  async _discoverIgAccount(userToken) {
    // Get Facebook Pages managed by this user, including their IG business accounts
    const url = `${GRAPH_URL}/me/accounts?fields=id,name,access_token,instagram_business_account{id,username}&access_token=${userToken}`;
    ...
    if (!page) {
      throw Object.assign(
        new Error('Nenhuma página do Facebook com conta Instagram Business vinculada foi encontrada. Vincule sua conta Instagram à sua página do Facebook primeiro.'),
        { status: 400 }
      );
    }
